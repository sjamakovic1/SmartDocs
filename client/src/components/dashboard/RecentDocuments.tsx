      ...
      <div className="mt-4 overflow-hidden rounded-lg border border-slate-200">
        <div className="divide-y divide-slate-100 md:hidden">
          {recentDocuments.map((document) => (
            <RecentDocumentMobileCard document={document} key={document.id} />
          ))}
        </div>
        <div className="hidden overflow-x-auto md:block">
          <table className="w-full min-w-[680px] border-collapse text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {recentDocuments.map((document) => (
                <RecentDocumentTableRow document={document} key={document.id} />
              ))}
            </tbody>
          </table>
        </div>
...
function RecentDocumentMobileCard({ document }: { document: Document }) {
  return (
...
function RecentDocumentTableRow({ document }: { document: Document }) {
  return (
